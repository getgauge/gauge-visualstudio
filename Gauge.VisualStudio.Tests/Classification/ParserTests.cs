...
namespace Gauge.VisualStudio.Tests.Classification
{
    ...
    public class ParserTests
    {
        ...
        [Test]
        public void ShouldGetStepName()
        {
            var stepText = "* Say hello to gauge";
            Assert.AreEqual(stepText, Parser.StepRegex.Match(stepText).Value.Trim());

            stepText = "Say hello to gauge";
            Assert.AreEqual("", Parser.StepRegex.Match(stepText).Value.Trim());

            stepText = "* say \"hello\" to \"gauge\" \n \n  ";
            Assert.AreEqual("* say \"hello\" to \"gauge\"", Parser.StepRegex.Match(stepText).Value.Trim());

            stepText = "* say \"hello\" and <adjka> to all";
            Assert.AreEqual(stepText, Parser.StepRegex.Match(stepText).Value.Trim());

            stepText = "* say !@ to all";
            Assert.AreEqual(stepText, Parser.StepRegex.Match(stepText).Value.Trim());
            
            stepText = "* Say \"{hi | bye / how  who ? <>=+-_)(*&^%$#@!~`}\" to \"[hello)\"";
            Assert.AreEqual(stepText, Parser.StepRegex.Match(stepText).Value.Trim());
